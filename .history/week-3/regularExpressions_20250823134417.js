...
//[a-z] looks for any lowercase character starting with a to z
...
const regex = /[abc]/; 
const result = regex.exec(" A hell nah girl!");
...
//you can even dictate how many times it repeats eg; \w{}\s
...
const patterns = {
    ...
    password:/^[\w@_-]{8,20}$/,
    slug: /^[a-z\d-]{8,20}$/,
    //emails have 3/4 parts; (yourname)@(domain).(extension)(.another extension)
    email: /^([a-z\d\.-])@([a-z\d-]+)\.([a-z]{2,8})(\.[a-z]{2,8})?$/    //now the dot(.) has a \ before so that we can escape and use it as a regular . instead f its meaning in js regular expressions.
    //Also we put a ? after the second extension cause it's optional.
}
