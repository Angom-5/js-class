...
inputs.forEach((input) => {
    input.addEventListener("keyup", (e) => {    //keyup tell the computer when a key on the keyboard is being pressed hence it listens to what kind of key that is
        e  console.log(e).target.attribute.name.valu      // target= for identifying the element, attribute=the attribute for that element, name=the nme and the value
    })
})

//now we have to test our thing to see that its valide. We use a function
function validate(field, regex) {
    console.log(regex.test(field.value));   // the way to test a regex is by regex.test(value of whatever you are testing)
}
